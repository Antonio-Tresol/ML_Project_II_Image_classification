...
class BilateralFilter(ImageTransformation):
    def __init__(self, d: int = 9, sigmaColor: int = 75, sigmaSpace: int = 75) -> None:
        """
    ...
    def fit(self, image: Image.Image) -> Image.Image:
        ...
        filtered_image = cv2.bilateralFilter(numpy_image, d=self.d, sigmaColor=self.sigmaColor, sigmaSpace=self.sigmaSpace)

        # Convert the filtered NumPy array back to PIL image
        filtered_pil_image = Image.fromarray(cv2.cvtColor(filtered_image, cv2.COLOR_BGR2RGB))
